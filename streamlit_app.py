...
try:
    if "supabase" not in st.secrets or "google_oauth" not in st.secrets:
        st.error("⚠️ Thiếu cấu hình secrets.toml (yêu cầu [supabase] và [google_oauth]).")
        st.stop()
    ...
    # Lấy thông tin OAuth từ secrets
...
def get_drive_service():
    """Xác thực Google Drive bằng OAuth 2.0 Refresh Token"""
    try:
        # Tái tạo Credentials từ thông tin trong secrets
        creds = Credentials(
...
def upload_to_drive_structured(file_obj, path_list, file_name):
    """Upload file vào thư mục cụ thể (Structured)"""
...
def upload_to_drive_simple(file_obj, sub_folder, file_name):
    """Upload đơn giản vào 1 folder chỉ định (Simple)"""
    srv = get_drive_service()
    if not srv: return "", ""
    try:
...
def download_from_drive(file_id):
    """Tải nội dung file về bộ nhớ đệm"""
    srv = get_drive_service()
...
def to_float(val):
    """Xử lý chuỗi tiền tệ về float an toàn"""
...
def strict_match_key(val):
    """Làm sạch chuỗi để matching [cite: 175]"""
    if val is None: return ""
    s = str(val).lower()
    return re.sub(r'\s+', '', s)

def calc_eta(order_date_str, leadtime_val):
    """Tính toán ETA [cite: 184]"""
    try:
...
# =============================================================================
# 3. LOGIC TÍNH TOÁN CORE (QUOTATION) 
# =============================================================================
...
def parse_formula(formula, buying_price, ap_price):
    """Xử lý công thức người dùng nhập [cite: 89-93]"""
...
# --- TAB 3: BÁO GIÁ (CORE FEATURE) [cite: 69-118] ---
with t3:
    ...
    if not st.session_state.quote_df.empty:
        ...
        # DISPLAY MAIN EDITOR [cite: 101-103]
        ...
        # --- VIEW TOTAL ROW (UPDATED LAYOUT) --- [cite: 104-107]
