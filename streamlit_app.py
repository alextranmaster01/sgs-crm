...
# 1. CẤU HÌNH & KHỞI TẠO
# =============================================================================
APP_VERSION = "V6044 - FINAL STABLE VERSION"
...
with t3:
        ...
        if st.button("Apply AP Price", key="btn_ap"):
            if not st.session_state.quote_df.empty and ap_f:
                ...
                for idx, row in df_copy.iterrows():
                    ...
                    df_copy.at[idx, "AP price(VND)"] = fmt_float_2(new_ap)
        ...
        if st.button("Apply Unit Price", key="btn_unit"):
            if not st.session_state.quote_df.empty and unit_f:
                ...
                for idx, row in df_copy.iterrows():
                    ...
                    df_copy.at[idx, "Unit price(VND)"] = fmt_float_2(new_unit)
    ...
    if not st.session_state.quote_df.empty:
        ...
        # Format tất cả các cột số với định dạng nghìn, chục, trăm, triệu
        cols_to_format = ["Buying price(RMB)", "Total buying price(rmb)", "Buying price(VND)", 
        ...
        for col in cols_to_format:
            if col in st.session_state.quote_df.columns:
                st.session_state.quote_df[col] = st.session_state.quote_df[col].apply(lambda x: fmt_num(x) if isinstance(x, (int, float, str)) else x)
        ...
        # --- THÊM DÒNG TỔNG (TOTAL ROW) ---
        df_display = df_show.copy()
        
        # Tính tổng cho các cột cần thiết
        ...
        total_row = {"Delete": False, "Cảnh báo": "TỔNG", "No": "TOTAL", "Item code": "", "Item name": "", "Specs": "", "Supplier": "", "Leadtime": ""}
        
        for c in cols_to_sum:
            if c in df_display.columns:
                try:
                    # Lấy giá trị số thực để tính tổng
                    numeric_vals = df_display[c].apply(lambda x: to_float(x) if c != "Exchange rate" else to_float(x))
        ...
        # Hiển thị Data Editor với chức năng xóa dòng
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Buying price(RMB)": st.column_config.TextColumn("Buying(RMB)", disabled=False),
                "Buying price(VND)": st.column_config.TextColumn("Buying(VND)", disabled=False),
                "Exchange rate": st.column_config.NumberColumn("Rate", format="%.3f"),
                "AP price(VND)": st.column_config.NumberColumn("AP Price", format="%.2f"),
                "Unit price(VND)": st.column_config.NumberColumn("Unit Price", format="%.2f"),
                "End user(%)": st.column_config.NumberColumn("End User%", format="%.2f"),
                "Buyer(%)": st.column_config.NumberColumn("Buyer%", format="%.2f"),
                "Import tax(%)": st.column_config.NumberColumn("Tax%", format="%.2f"),
                "VAT": st.column_config.NumberColumn("VAT", format="%.2f"),
                "Transportation": st.column_config.NumberColumn("Transport", format="%.2f"),
                "Management fee(%)": st.column_config.NumberColumn("Mgmt%", format="%.2f"),
                "Payback(%)": st.column_config.NumberColumn("Payback%", format="%.2f"),
            },
        ...
        if 'Delete' in edited_df.columns:
            ...
            if not rows_to_delete.empty:
                ...
                if rows_to_delete_indices:
                    if st.button(f"🗑️ Xóa {len(rows_to_delete_indices)} dòng đã chọn", key="btn_delete_rows"):
            ...
            for idx, row in df_data_only.iterrows():
                if idx < len(st.session_state.quote_df):
                    for col in edited_df.columns:
                        if col in st.session_state.quote_df.columns and col != "No":
                            st.session_state.quote_df.at[idx, col] = row[col]
        ...
        if st.session_state.get('show_review', False):
            ...
            total_row_review = {"No": "TOTAL", "Item code": "", "Item name": "", "Specs": "", 
                               "Q'ty": fmt_num(total_qty_review), 
                               "Unit price(VND)": fmt_num(total_unit_review), 
                               "Total price(VND)": fmt_num(total_price_review), 
                               "Leadtime": ""}
            
            df_review = pd.concat([df_review, pd.DataFrame([total_row_review])], ignore_index=True)
            
            # Hiển thị với style cho dòng tổng
            ...
            st.markdown(f'<div class="total-view">💰 TỔNG CỘNG: {fmt_num(total_price_review)} VND</div>', unsafe_allow_html=True)
