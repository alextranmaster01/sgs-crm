...
# CẤU HÌNH & VERSION
# =============================================================================
APP_VERSION = "V4859 - FINAL FIX (RESTORED UPSERT ALGO)"
...
# --- DB HANDLERS (UPDATED TO APP-(3) LOGIC) ---
@st.cache_data(ttl=5) 
def load_data(table):
    try:
...
# HÀM LƯU DỮ LIỆU CHUẨN (UPSERT - KHÔNG XÓA DỮ LIỆU)
def save_data(table, df, unique_cols=None):
    ...
    try:
        ...
        # Đổi tên cột nếu DF đến từ Tab Báo giá
        ...
        # LOGIC UPSERT CỦA APP-(3)
        if unique_cols:
            conflict_target = ",".join(unique_cols)
            # Chia nhỏ batch để tránh lỗi payload too large
            chunk_size = 500
...
# --- UI ---
st.title("HỆ THỐNG CRM QUẢN LÝ (V4859)")
...
with t2:
    ...
    with c1:
        ...
        if up_file and st.button("🚀 IMPORT & TÍNH TOÁN"):
            try:
                # SỬ DỤNG LẠI LOGIC CŨ (HEADER 0 & HEADER MAPPING)
                ...
                # QUAN TRỌNG: DÙNG UPSERT (save_data) THAY VÌ OVERWRITE ĐỂ KHÔNG MẤT DỮ LIỆU
                # Sử dụng item_code + price làm key để tránh đè nếu trùng code nhưng khác giá
                save_data("crm_purchases", pd.DataFrame(rows), unique_cols=['item_code', 'buying_price_rmb'])
    ...
    if 'quote_result' in st.session_state and not st.session_state.quote_result.empty:
        ...
        with st.container(border=True):
            ...
            if st.button("💾 Lưu vào Lịch sử"):
                ...
                save_data("crm_shared_history", to_save)
                st.success("Đã lưu!")
