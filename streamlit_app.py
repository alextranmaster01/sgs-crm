...
with t5:
    ...
    with t5_1:
        ...
        with c_debug:
            with st.expander("🛠️ KIỂM TRA LỖI (Tại sao đơn không qua Lịch sử?)"):
                st.write("**Danh sách PO ĐÃ THANH TOÁN:**")
                st.info(", ".join(debug_paid_info) if debug_paid_info else "Chưa có.")
        ...
        if not df_pay.empty:
            ...
            with c_p_list:
                st.dataframe(
                    df_pay,
                    column_config={
                        "proof_image": st.column_config.ImageColumn("Proof Image"),
    ...
    with t5_3:
        ...
        if not df_history.empty:
            ...
            # ---------------- PHẦN 1: NCC -> LỊCH SỬ ĐƠN HÀNG (TRACKING) ----------------
            st.markdown("### 📦 Lịch sử Đơn Hàng (Tracking - NCC)") 
            # (Đã tách riêng, chỉ hiển thị NCC ở đây)
            if not df_hist_ncc.empty:
                st.dataframe(
                    df_hist_ncc,
                    column_config={
                        "proof_image": st.column_config.ImageColumn("Proof"), # 1. Hiện ảnh
                        "status": st.column_config.TextColumn("Status"),
                        "po_no": "PO No.", 
                        "partner": "Nhà Cung Cấp", 
                        "eta": "ETA",
                        "payment_status": st.column_config.TextColumn("Trạng thái tiền") # 2. Cột trạng thái tiền
                    },
                    use_container_width=True, hide_index=True
                )
            else:
            ...
            # (Đơn hàng KH đánh dấu số 1 trong ảnh sẽ tự động chuyển xuống đây do logic lọc)
            ...
            if not df_hist_kh.empty:
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "partner": "Khách Hàng", 
                        "eta": "ETA",
                        "payment_status": st.column_config.TextColumn("Trạng thái tiền") # 3. Cột trạng thái tiền
                    },
                    use_container_width=True, hide_index=True
                )
            else:
