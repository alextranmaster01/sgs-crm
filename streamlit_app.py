...
def parse_formula(formula, buying_price, ap_price):
    ...
    # 3. Replace Keywords
    # Hỗ trợ: BUY (Buying Price VND), AP (AP Price VND), BUYING, AP_PRICE
    ...
    s = s.replace("AP_PRICE", str(val_ap))
    s = s.replace("AP", str(val_ap))
    
    # 4. Thay thế dấu phẩy thành chấm
    s = s.replace(",", ".")
    
    # 5. Cho phép các ký tự toán học hợp lệ
...
# --- HÀM ĐỊNH DẠNG SỐ (dùng chung với tab Kho hàng) ---
def format_number_column(series):
    """Định dạng số với dấu phẩy ngăn cách hàng nghìn và 1 chữ số thập phân"""
    def format_cell(x):
        try:
            ...
            return str(x)
    return series.apply(format_cell)
    ...
    with c_form1:
        ...
        if st.button("Apply AP Price"):
            if not st.session_state.quote_df.empty:
                ...
                st.success("✅ Đã áp dụng công thức AP Price!")
                st.rerun() 
    ...
    with c_form2:
        ...
        if st.button("Apply Unit Price"):
            if not st.session_state.quote_df.empty:
                ...
                st.session_state.quote_df = recalculate_quote_logic(st.session_state.quote_df, {})
                st.success("✅ Đã áp dụng công thức Unit Price!")
                st.rerun() 
        st.markdown('</div>', unsafe_allow_html=True)
    
    if not st.session_state.quote_df.empty:
        # Tạo bản sao để hiển thị định dạng
        df_display = st.session_state.quote_df.copy()
        
        # Định dạng các cột số
        numeric_cols = ["Q'ty", "Buying price(RMB)", "Total buying price(rmb)", "Exchange rate",
        ...
        for col in numeric_cols:
            if col in df_display.columns:
                df_display[col] = df_display[col].apply(lambda x: f"{to_float(x):,.1f}" if to_float(x) != 0 else "0.0")
        ...
        for col in numeric_cols:
            if col in st.session_state.quote_df.columns:
                total_row[col] = f"{st.session_state.quote_df[col].apply(to_float).sum():,.1f}"
        
        # Thêm dòng tổng vào DataFrame hiển thị
        df_display_with_total = pd.concat([df_display, pd.DataFrame([total_row])], ignore_index=True)
        ...
        column_config["Xóa"] = st.column_config.CheckboxColumn(
            "Xóa", 
            help="Chọn để xóa dòng này",
            default=False
        )
        
        # Cột số
        for col in ["No", "Q'ty"]:
            if col in df_display_with_total.columns:
                column_config[col] = st.column_config.NumberColumn(col, format="%d")
        
        # Cột tiền tệ (1 chữ số thập phân)
        ...
        for col in money_cols:
            if col in df_display_with_total.columns:
                column_config[col] = st.column_config.NumberColumn(
                    col,
                    format="%.1f",
                    help="Nhấn để chỉnh sửa"
                )
        
        # Cột Exchange rate
        if "Exchange rate" in df_display_with_total.columns:
            column_config["Exchange rate"] = st.column_config.NumberColumn("Exchange rate", format="%.2f")
        
        # Cột cảnh báo
        column_config["Cảnh báo"] = st.column_config.TextColumn("Cảnh báo", disabled=True)
        
        # Cột hình ảnh
        if "Image" in df_display_with_total.columns:
            column_config["Image"] = st.column_config.ImageColumn("Image")
        
        # Hiển thị data editor
        edited_df = st.data_editor(
            df_display_with_total,
        ...
        # Xử lý khi người dùng chỉnh sửa
        if not edited_df.equals(df_display_with_total):
            # Lấy dữ liệu đã chỉnh sửa (loại bỏ dòng tổng)
            edited_data = edited_df[edited_df["No"] != "TOTAL"].copy()
            
            # Chuyển đổi dữ liệu định dạng trở lại số
            for col in numeric_cols:
                if col in edited_data.columns:
                    edited_data[col] = edited_data[col].apply(to_float)
            
            # Cập nhật session state
            st.session_state.quote_df = recalculate_quote_logic(edited_data, {})
            st.success("✅ Đã cập nhật dữ liệu!")
            st.rerun()
        
        # Xóa các dòng đã chọn
        if st.button("🗑️ Xóa dòng đã chọn"):
            # Lọc các dòng không được chọn để xóa
            keep_rows = edited_df[edited_df["Xóa"] == False]
            keep_rows = keep_rows[keep_rows["No"] != "TOTAL"]  # Giữ lại dòng tổng
            
            if len(keep_rows) < len(st.session_state.quote_df):
                # Cập nhật số thứ tự
                keep_rows = keep_rows.copy()
                keep_rows["No"] = range(1, len(keep_rows) + 1)
                
                # Chuyển đổi lại số
                for col in numeric_cols:
                    if col in keep_rows.columns:
                        keep_rows[col] = keep_rows[col].apply(to_float)
                
                st.session_state.quote_df = recalculate_quote_logic(keep_rows, {})
                st.success(f"✅ Đã xóa {len(st.session_state.quote_df) - len(keep_rows)} dòng!")
                st.rerun()
            else:
                st.info("Không có dòng nào được chọn để xóa.")
        ...
        if st.session_state.get('show_review', False):
            ...
            if "Q'ty" in df_review.columns:
                df_review["Q'ty"] = df_review["Q'ty"].apply(lambda x: f"{to_float(x):,.0f}")
            if "Unit price(VND)" in df_review.columns:
                df_review["Unit price(VND)"] = df_review["Unit price(VND)"].apply(lambda x: f"{to_float(x):,.1f}")
            if "Total price(VND)" in df_review.columns:
                df_review["Total price(VND)"] = df_review["Total price(VND)"].apply(lambda x: f"{to_float(x):,.1f}")
            ...
            if "Q'ty" in st.session_state.quote_df.columns:
                total_rev["Q'ty"] = f"{st.session_state.quote_df['Q\'ty'].apply(to_float).sum():,.0f}"
            if "Unit price(VND)" in st.session_state.quote_df.columns:
                total_rev["Unit price(VND)"] = f"{st.session_state.quote_df['Unit price(VND)'].apply(to_float).sum():,.1f}"
            if "Total price(VND)" in st.session_state.quote_df.columns:
                total_rev["Total price(VND)"] = f"{st.session_state.quote_df['Total price(VND)'].apply(to_float).sum():,.1f}"
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "Q'ty": st.column_config.TextColumn("Q'ty"),
                    "Unit price(VND)": st.column_config.TextColumn("Unit price(VND)"),
                    "Total price(VND)": st.column_config.TextColumn("Total price(VND)")
                }
            )
        ...
        with c_sv:
            ...
            if st.button("💾 LƯU LỊCH SỬ (QUAN TRỌNG ĐỂ LÀM PO)"):
                if cust_name:
                    ...
                    params = {}  # Khởi tạo params để tránh lỗi
                    clean_params = {}
