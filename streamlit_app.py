...
def recalculate_quote_logic(df, params):
    ...
    # QUAN TRỌNG: CỘNG TỔNG CHI PHÍ TỪ CÁC CỘT (Đã cho phép sửa tay)
    # Không dùng params để ghi đè ở đây, để tôn trọng giá trị người dùng nhập
...
with t3:
    ...
    if not st.session_state.quote_df.empty:
        ...
        # Combine: Add Total Row to the end of Main Data
        ...
        # Configure columns (using %.1f to enforce 1 decimal place)
        column_config = {
        ...
        if st.session_state.get('show_review', False):
            ...
            # Apply FMT_NUM to review table to look like Kho Hang
